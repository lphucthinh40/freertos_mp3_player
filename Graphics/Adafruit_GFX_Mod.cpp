...
void Adafruit_GFX::writeln(str string)
{
    uint8_t start_at = 0;
    if (string[1] == ':') start_at = 2;
    for (uint8_t i = start_at; i < string.getLen(); i++)
    {
        if (string[i]=='.') break;
        write(string[i]);
    }
...
*/
/**************************************************************************/
void Adafruit_GFX::getTextBounds(const str &string, int16_t x, int16_t y,
        int16_t *x1, int16_t *y1, uint16_t *w, uint16_t *h) {
    if (string.getLen() != 0) {
        getTextBounds(const_cast<char*>(string.c_str()), x, y, x1, y1, w, h);
    }
}
...
void Adafruit_GFX_Button::drawButton(bool inverted) {
  ...
  for (uint8_t i = 0; i < 10; i++) _gfx->write(_label[i]);
}
