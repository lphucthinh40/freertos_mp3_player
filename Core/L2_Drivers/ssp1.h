...
// ***********ADDED FUNCTION FOR MP3 LAB*************
static inline void ssp1_format(uint8_t len, uint8_t mode)
{
    LPC_SSP1->CR0 &= ~(0xFF); // Clear first 8 bits ([0:3] len, [4:5] frame format, [6:7] mode)
    LPC_SSP1->CR0 |= ((len - 1) << 0) + (mode << 6);
}

// MSB first
static inline char ssp1_exchange_byte16(uint16_t out)
{
    uint16_t data;
    data = ssp_exchange_byte(LPC_SSP1, out & 0xFF);
    data = (data << 8) + ssp_exchange_byte(LPC_SSP1, out >> 8);
    return data;
}
